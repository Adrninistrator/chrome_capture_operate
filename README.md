...
2. 在 Chrome 插件中配置 Cookie、Authorization 推送规则（默认"全部禁止"，需改为允许推送目标网站）
...
8. 在日常使用的 Chrome 中登录目标网站，插件自动向 Python 服务推送 Cookie、Authorization 供脚本使用
...
| 执行脚本 | 日常使用的Chrome | 插件自动推送网站Cookie/Authorization到长期运行的Python脚本 | 需要（从长期运行的Python脚本获取） | 无（可手工或定时自动执行） | 业务结果与执行日志 |

## 实现机制说明

- **Cookie、Authorization 自动供给**：插件安装在日常使用的 Chrome 中，按启动 / Cookie 与 Authorization 变更 / 定时三种时机自动推送到 Python 服务，推送范围可配置；Cookie 与 Authorization 仅存内存不落盘。固化脚本运行时通过 HTTP 接口获取，始终拿到最新的登录态，无需人工复制 Cookie。
...
- **Chrome 插件**（安装在日常使用的 Chrome）：自动将浏览器 Cookie 与请求头中的 Authorization 推送给 Python 服务，让脚本以登录态发起请求
...
| Chrome Cookie接收状态 | 查看最近 50 次插件 Cookie、Authorization 推送记录（含查看推送详情） |
...
执行阶段（脚本自动执行）：
  日常Chrome ──插件自动推送Cookie/Authorization──> Python服务（长期运行）
  固化脚本（手工/定时执行）──获取Cookie/Authorization──> Python服务
  固化脚本 ──携带Cookie/Authorization访问──> 目标网站
```
...
4. 在插件页面配置参数：
   - **推送目标地址**、**Cookie推送时间间隔**（Authorization 变更同样按此间隔）：通常不需要修改，保持默认值即可
   - **Cookie推送范围**（Authorization 的推送同样受此范围控制）：**必须按需修改**——默认为"全部禁止"，不会向 Python 服务推送任何 Cookie 与 Authorization；需改为"全部允许"或"按指定范围推送"（允许与不允许清单，支持 `*.baidu.com` 通配符，不允许优先）
...
- **Python脚本**（如 `main.py`）：固化脚本本体，运行时通过 HTTP 接口获取 Cookie 与 Authorization 后重放请求
...
**前提：Cookie 推送设置**（脚本以登录态访问邮箱，依赖插件推送 Cookie；Authorization 走同一推送链路）
...
- **接收记录**：展示最近 50 次插件 Cookie 与 Authorization 推送（Chrome实例编号/时间/原因/地址/数量/成功/失败），与插件"推送记录"TAB 展示的内容一致；页面顶部显示当前内存中的 Cookie 总数与最近推送时间
- **查看推送详情**：点击某条记录的**查看**展开详情——实例编号、Cookie数量与Authorization数量、**Cookie对应的地址（域名或IP）**、**Authorization所属地址**与 Cookie key，用于确认某次推送覆盖了哪些网站、是否包含目标网站的会话 Cookie
...
需要在同一个网站登录多个账号时（如管理多个账号的工单/邮箱），为每个账号创建一个独立的 Chrome 实例分别登录——各实例的数据互不干扰，固化脚本可按实例编号取用对应账号的 Cookie 与 Authorization。
...
- 该 Chrome 之后直接登录账号即可，Cookie/Authorization 推送自动归到对应实例编号
- 固化脚本获取指定账号的 Cookie 与 Authorization：查询接口带 `profile` 参数（见 `api/api.md`）；不带则查所有实例
...
| 脚本执行提示未获取到 Cookie / 报"当前没有任何 Cookie" | 依次检查：Chrome 插件是否已安装；插件推送范围是否已从"全部禁止"改为其他模式；在"Chrome Cookie接收状态"页查看是否有接收记录；点击**查看**可展开该次推送的Cookie对应的地址（域名或IP）、Authorization所属地址、Cookie数量与Cookie的key——若无记录说明插件未推送，可打开插件页面点击**立即推送一次**人工推送 |
| 脚本执行返回未登录 / 401 | 在"Chrome Cookie接收状态"页确认插件已推送且包含会话 Cookie（或对应主机的 Authorization）；确认登录态与插件在同一个 Chrome 浏览器进程中；推送范围 allow 清单写 `*.example.com`（带 `*.`）才能命中裸域与全部子域；推送正常仍 401 多为会话过期——在插件**会话保持**标签页配置定时刷新对应网站页面（见「Chrome插件的其他功能」一节） |
